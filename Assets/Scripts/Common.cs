...
{
    public static float SinT(float value, bool clamp = true)
    {
        value = clamp ? Mathf.Clamp(value, 0f, 1f) : value;
    ...
    public static float EitherOr(float either, float or)
    {
        return Random.Range(0, 2) == 1 ? either : or;
    }

    public static bool EitherOrBool() => Random.Range(0, 2) == 1;
    ...
    static public WaitForKeyDownInstruction WaitForKeyDown(KeyCode key)
    {
        return new WaitForKeyDownInstruction(key);
    }
    public class WaitForKeyDownInstruction : CustomYieldInstruction
    {
        KeyCode kc;
        public WaitForKeyDownInstruction(KeyCode kc) => this.kc = kc;
        public override bool keepWaiting => !Input.GetKeyDown(kc);
    }
    ...
    {
        Vector2 ViewportPosition = canvas.worldCamera.WorldToViewportPoint(world_position);
...
public static class ListExtensions
{
    ...
    {
        T ret = list[Random.Range(0, list.Count)];
        if (remove)
        {
            list.Remove(ret);
        }
