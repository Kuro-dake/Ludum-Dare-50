...
public class AudioManager : MonoBehaviour
{
    ...
    public AudioSource PlayTrack(AudioClip ac, float volume = 1f, FloatRange pitch = null)
    {
        ...
        foreach (AudioSource asc in transform.GetComponents<AudioSource>())
        {
            if (!asc.isPlaying)
            {
                Destroy(asc);
        ...
        if (c_as == null && transform.GetComponents<AudioSource>().Length < max_sources)
        {
            c_as = transform.gameObject.AddComponent<AudioSource>();
        }
        if (c_as != null)
        {
            c_as.clip = ac;
